...
export async function GET() {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: '연락처 정보를 조회할 수 없습니다.' }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 필수 필드 검증
    const { wedding_info_id, side, role, full_name, contact_type, contact_value } = body;

    if (!wedding_info_id || !side || !role || !full_name || !contact_type || !contact_value) {
    ...
    const { data, error } = await supabase.from('wedding_contacts').insert(body).select().single();

    if (error) {
      console.error('연락처 정보 생성 실패:', error);
      return NextResponse.json({ error: '연락처 정보를 생성할 수 없습니다.' }, { status: 500 });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: '수정할 연락처 정보의 ID가 필요합니다.' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: '연락처 정보를 수정할 수 없습니다.' }, { status: 500 });
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: '삭제할 연락처 정보의 ID가 필요합니다.' }, { status: 400 });
    }

    const supabase = await createClient();

    const { error } = await supabase.from('wedding_contacts').delete().eq('id', id);

    if (error) {
      console.error('연락처 정보 삭제 실패:', error);
      return NextResponse.json({ error: '연락처 정보를 삭제할 수 없습니다.' }, { status: 500 });
    }
